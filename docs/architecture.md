...
| Client Query cache | Messages/threads/profiles/member lists | Different result shapes and partly unscoped identity; broad invalidation | Targeted invalidation; clear in-memory entries on logout/session actor change; message timeline/thread/detail/pin/bookmark keys include actor/workspace/resource; action invalidation is actor/workspace/channel-bounded; channel bookmark reads are server-filtered; member-list keys include actor/workspace/channel; profile and card keys include viewer/workspace/target; profile/card server reads verify requester and target membership; continue per-key revocation audit |
