...
| Client Query cache | Messages/threads/profiles | Different result shapes and mostly unscoped identity; broad invalidation | Targeted invalidation; clear in-memory entries on logout/session actor change; include workspace scope in profile reads; continue per-key actor-scope and revocation audit |
