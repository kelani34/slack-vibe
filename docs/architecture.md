...
Interactive message sends now also carry a stable client intent through the composer, optimistic cache and server action. PostgreSQL enforces one `(userId, clientMutationId)` row; a private request hash detects payload conflicts and is globally omitted from message reads. The public send action requires a valid UUID, checks current membership before returning a canonical retry result, and reconciles concurrent requests through the unique constraint. File upload now follows a separate owner/channel-bound signed-intent path; send accepts only finalized one-use upload intent IDs and derives attachment metadata from PostgreSQL. The hook preserves valid uploaded intent IDs for in-page retry and requests a fresh transfer after expiry. Other message writers, durable offline recovery, resumable transfer and orphan cleanup need their own completion evidence.
...
The initial application/DB stack is retained. Managed media and selected processors are purposeful integrations for requested features, not a rewrite into microservices. Provider selection and measured limitations determine any further infrastructure. W09 now has optional attachment object locators, `UploadIntent` ownership/finalization state, a direct browser-to-private-storage upload token, one-use message attachment, and a server read-grant boundary that rechecks membership and message visibility before issuing five-minute preview/download URLs. `MessageItem` requests reads only when a private attachment is opened. TUS progress, pause/resume and cancel/remove are implemented; a daily Vercel Cron route removes at most 100 expired, unattached private objects after a one-hour grace period, then retires their intents. This remains partial: the private bucket/provider and `CRON_SECRET` are not provisioned or verified, the legacy profile-avatar path still uses the public bucket, and cleanup has only mocked-provider tests.
...
| Client Query cache | Messages/threads/profiles/member lists/scheduled drafts | Different result shapes and partly unscoped identity; broad invalidation | Targeted invalidation; clear in-memory entries on logout/session actor change; message timeline/thread/detail/pin/bookmark and scheduled-draft keys include actor/workspace/resource; private timeline and thread reads stay disabled until actor identity is known; channel bookmark and scheduled-draft reads are server-filtered; member-list keys include actor/workspace/channel; profile/card keys include viewer/workspace/target; profile/card server reads verify requester and target membership; continue per-key revocation audit |
