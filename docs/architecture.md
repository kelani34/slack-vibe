...
| Client Query cache | Messages/threads/profiles | Different result shapes and mostly unscoped identity; broad invalidation | Targeted invalidation; clear in-memory entries on logout/session actor change; continue per-key actor-scope and revocation audit |
