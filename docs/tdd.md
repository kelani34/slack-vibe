...
**W09 cleanup red/green, 26 September 2026:** route tests first failed because the cleanup handler did not exist; they now verify absent/wrong/unconfigured CRON_SECRET denial, a hard 100-item cap, private-bucket/no-attachment filtering, one-hour expiry grace, no-work short-circuit, Storage-before-database ordering, and retryable provider/database failures. PostgreSQL index coverage first failed without the cleanup expiry index, then passed after adding the index and additive migration. Vercel config registers the route once daily. Supabase remains mocked; live provider semantics, secret provisioning, concurrent invocations and independent QA remain open.
