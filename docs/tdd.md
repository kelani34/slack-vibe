...
Current commands are `test` (the default Vitest projects), `test:unit`, `test:component`, `test:integration`, `test:coverage` and the opt-in `test:performance`. `.github/workflows/ci.yml` runs the three default test projects, Prisma generation, typecheck, production build and full lint. The unit, component and isolated PostgreSQL integration suites pass separately, as do typecheck, build and lint; the workflow has no hosted run yet. The full-repository lint baseline is now clean locally, down from 84 errors and 32 warnings. Future lanes `test:contracts`, `test:policies`, `test:e2e`, `test:mobile`, `test:a11y`, `test:migrations` and `test:ci` remain to be added with their real environments and failure-propagation checks.
