...
Private-channel files must not be placed behind permanent public URLs. New message attachments use private object keys scoped to channel and upload intent; legacy URL-based attachment and avatar paths remain to be migrated. A server authorizes a short-lived upload or download grant and validates finalized object ownership. The service role bypasses ordinary policies and is therefore a server-only trust boundary; Supabase documents this in [Storage access control](https://supabase.com/docs/guides/storage/security/access-control).

**Current W09 evidence:** an earlier signature-check slice rejected SVG and checked the first 12 bytes for supported image, video, audio and PDF MIME types before its then-current server-buffer upload path; red/green integration cases covered mismatched and matching PNG/MP4 headers plus an SVG payload. These are only prefix checks for supported file types. PR #16 hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. Both hosted CI runs ([36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472), [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785)), Vercel and GitGuardian passed; CodeRabbit review was disabled on the stacked base. Legacy profile-avatar upload and existing public URL records remain. A06 remains open. The attachment table now has an optional unique bucket/path pair. The grant action rechecks membership and message visibility before creating separate inline-preview and forced-download URLs with a 300-second expiry. Supabase documents [`createSignedUrl`](https://supabase.com/docs/reference/javascript/file-buckets-createsignedurl) and notes that issued signed URLs remain valid until expiry; current authorization blocks future grants after membership loss, but cannot revoke an already-issued URL. W09 now creates two-hour signed upload tokens for owner/channel-scoped random paths, uploads directly from the browser, and verifies the stored object before changing its intent to UPLOADED. The message action accepts only those intent IDs, confirms the current actor/channel, expiry, one-use attachment relation, five-file limit and 25 MB aggregate limit, and copies metadata from the server record. Attachment rows with private locators cannot also contain a URL. Media finalization compares stored size/MIME and checks the first 12 bytes through a short signed range read; this rejects known signature mismatches but does not fully decode containers, scan files, or validate Office bytes. The private bucket must be provisioned as non-public with the matching 10 MB per-file limit and MIME allowlist before uploads can work. Signed upload tokens are bearer credentials valid for up to two hours; current authorization is rechecked during finalization and send. The new `/api/cron/cleanup-uploads` route requires the configured `CRON_SECRET`, applies a one-hour expiry grace, filters out attachment-referenced objects, caps each run at 100 paths, deletes through the Storage API before database rows, and returns retryable failures without exposing provider errors. Its daily Vercel schedule and expiry index are in source; the private bucket, secret, real provider behavior and live cron invocation are not verified. Direct browser transfer avoids routing file bodies through Next/Vercel functions. The browser avatar uploader, live provider/device verification and independent QA remain open.

Declared MIME/name/size are untrusted. Validate object bytes/metadata and prevent executable inline previews. Use image/document allowlists, safe download disposition, and limits before allocating whole buffers. TUS upload fingerprints are client-side resume hints only; they never authorize reuse. The server must issue or renew a grant only after checking the current actor's workspace and channel memberships, upload-intent state, expiry, bucket and unchanged object path. Keep the signed token out of logs and diagnostics, and never accept a client-selected fingerprint as proof of ownership. Broader arbitrary-file sharing requires a scanning/quarantine decision. Log object IDs and error categories rather than private file contents or signed URLs.
