...
**Current W09 evidence:** the server now rejects SVG and checks the first 12 bytes for supported image, video, audio and PDF MIME types before uploading; red/green integration cases cover mismatched and matching PNG/MP4 headers plus an SVG payload. These are only prefix checks for supported file types. PR #16 hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. Both hosted CI runs ([36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472), [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785)), Vercel and GitGuardian passed; CodeRabbit review was disabled on the stacked base. The bucket still returns permanent public URLs, the browser profile-avatar upload still bypasses the server action, Office document bytes are not inspected, and the server buffers the complete file. A06 remains open. The attachment table now has an optional unique bucket/path pair. The new grant action rechecks membership and message visibility before creating separate inline-preview and forced-download URLs with a 300-second expiry. Supabase documents [`createSignedUrl`](https://supabase.com/docs/reference/javascript/file-buckets-createsignedurl) and notes that issued signed URLs remain valid until expiry; current authorization blocks future grants after membership loss, but cannot revoke an already-issued URL. The configured private bucket and live provider behavior are unverified.
