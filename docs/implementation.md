...
**W05 profile-cache update, 25 September 2026:** Profile and hover-card reads now verify that both requester and target are current workspace members before returning profile data. Previously the hover-card action checked only target membership, so an authenticated outsider who knew a workspace and member ID could receive member contact details. An integration regression reproduced that leak, then verified that an outsider receives `null` while an authorized workspace member receives the intended card fields. Profile sidebar keys now include target, workspace, and viewing actor; hover-card keys include target, workspace, and viewer, and the card query stays disabled when the caller has no viewer identity. Component regressions first failed against the incomplete keys and pass with the full scope. Profile edits invalidate both profile and card key prefixes so all scoped copies refresh. At the profile-access checkpoint, the full local suite passed 278 tests across 60 files; current combined evidence is 289 tests across 61 files. Typecheck, lint, docs check and production build pass. This closes the hover-card requester-membership gap and prevents cross-viewer cache reuse; revocation while a card remains mounted, other profile/search/starred projections and independent QA remain open. PR #11 at `23ca9d4` passed hosted CI runs [36158693946](https://github.com/kelani34/slack-vibe/actions/runs/36158693946) and [36158731887](https://github.com/kelani34/slack-vibe/actions/runs/36158731887), plus CodeRabbit, GitGuardian and Vercel checks.

**W05 scheduled-draft access/cache update, 25 September 2026:** Composer schedule-list keys now include actor, workspace, channel and parent thread; absent actor identity disables the private query. The server requires current workspace membership and a channel record in that workspace with current channel membership before returning the actor's future rows. It selects only the fields used by the scheduled-message UI. Test-first component cases caught the old incomplete key and enabled-without-actor behavior; a database integration case reproduced the stale-channel-membership disclosure and now passes, and another asserts the narrow projection. The focused component/integration run passes 39 tests; the full suite passes 289 tests across 61 files, and typecheck, lint, docs validation and production build pass. Publication, scheduling race semantics, and workspace-wide scheduled-page scope remain separate W07/F28 work.

**W05 thread-query identity guard, 25 September 2026:** The thread sidebar already keys parent/reply data by actor, workspace and message scope, but it could execute those queries while the actor prop was absent under a shared fallback identity. A component regression failed on both query options before the fix; both now remain disabled until `currentUserId` is available. Normal actor-scoped reads and realtime behavior remain unchanged.

**W05 member-projection cache update, 25 September 2026:** The channel Members tab and channel-members dialog now key both member lists by actor, workspace, and channel. Two component tests failed against the previous keys and pass with the full scope; add/remove mutations invalidate their matching scoped entries. The server actions continue to authorize every read. Real membership-revocation propagation, other profile/search/starred projections, reconnect behavior, and measured production latency remain open.

**W05 conversation-cache update, 25 September 2026:** Timeline, parent-message and thread cache keys now include actor, workspace and channel/message scope. ChatPanel passes the authenticated actor and workspace through the composer, timeline, thread and retry surfaces; those identity props are required where a workspace-scoped query is constructed. A shared key helper keeps timeline reads, context restoration, thread reads/realtime refresh and optimistic send/retry updates aligned. Test-first regressions proved the prior timeline, parent and thread query keys were unscoped, then passed after propagation. PR #9 at `990f76f` passed both hosted test/type/build/lint runs, GitGuardian and Vercel. Server authorization remains mandatory; live permission revocation and real cross-tab convergence are open.

**W05 saved-message cache update, 25 September 2026:** Pinned and bookmarked conversation projections now use actor/workspace/channel-scoped keys, and pin/bookmark mutations invalidate only the matching cache. The conversation panel asks the server for bookmarks in its active channel instead of fetching the full user bookmark list and filtering it in the browser. Both loaders require current workspace and channel membership; a PostgreSQL regression verifies stale channel membership cannot expose pinned or bookmarked content after workspace removal. New component tests first failed on unscoped keys, broad bookmark invalidation and all-bookmark loading, then passed. At that checkpoint the full local suite passed 274 tests across 59 files; profile access raised it to 278 across 60, and the scheduled-access slice raised it to 282, and the thread viewer-identity guard raises it to 286 across 61. The browser-visible panel is unchanged. PR #10 at `09aa5ca` passed hosted CI runs [36156507989](https://github.com/kelani34/slack-vibe/actions/runs/36156507989) and [36156547919](https://github.com/kelani34/slack-vibe/actions/runs/36156547919), plus GitGuardian and Vercel checks.

**W05 cache-isolation update, 25 September 2026:** Selecting Log out now synchronously clears the current tab's React Query cache before Auth.js sign-out. A session boundary also clears it after the session changes to unauthenticated, navigates that tab to login with its local path preserved, or clears it and refreshes the server-rendered route when the authenticated actor ID changes. Component tests cover same-tab logout, propagated unauthenticated state/navigation and actor switch with a server refresh. This closes only client-memory query-cache cleanup at those transitions; real two-tab broadcast delivery, live transport teardown, every local/private store, revocation while a view remains mounted and independent QA remain open.

**Remaining W05 work:** finish remaining cache contracts; audit profile/member/search/starred-item list projections; measure scheduled-message polling and complete workspace-wide schedule-page scope; exercise the status-based catch-up and Auth.js session broadcast under real network loss and multi-tab load; trace first-page hydration and mounted-shell burst behavior; run production-build cold/warm timing and `EXPLAIN (ANALYZE, BUFFERS)` on fixed seeded fixtures; establish payload, request and query budgets; and verify cache invalidation after membership removal and concurrent read changes. W05 stays open until those measurements and independent QA are recorded.
...
**W09 partial progress, 25 September 2026:** TDD exposed arbitrary accepted media bytes labeled as PNG or MP4 and active SVG content declared as `image/svg+xml`. The server now checks the first 12 bytes for supported image, video, audio and PDF MIME types and rejects SVG before storage access. Integration tests reproduced mismatched PNG and MP4 failures and verify matching PNG and MP4 headers pass. PR #16 passed CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), Vercel and GitGuardian; CodeRabbit review was disabled for the stacked base. The check does not fully decode files, scan content, or verify Office document formats. Hosted CI passed both configured runs ([36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472), [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785)); Vercel and GitGuardian passed, while CodeRabbit review was disabled for this stacked base. Uploads still use the service-role server buffer and return permanent public URLs, the legacy profile uploader still uploads from the browser, and attachment ownership/finalization, private grants, signed reads, migration and cleanup remain W09 work. Do not treat this slice as closing A06/F29/F30.

**W09 authorized-read slice:** the Prisma migration adds optional `storageBucket`/`storagePath` references, requires them as a pair and prevents duplicate locators. `getAttachmentAccessUrls` requires an authenticated current channel member, excludes deleted and future-scheduled messages, enforces the channel-prefixed private path and issues separate inline-preview and forced-download URLs for 300 seconds. `MessageItem` makes private attachment tiles accessible buttons, avoids rendering their stored URL as a thumbnail, shows a reduced-motion-safe opening state, and retries after denial while URL-only legacy attachments keep working. The integration and component tests use a mocked storage provider. The full suite passes 300 tests across 62 files; typecheck, lint, documentation validation and isolated production build pass locally. The private bucket is not provisioned or confirmed, upload still writes public URLs, the five-minute URL cannot be revoked before expiry, and independent browser/device QA has not run. This does not close A06/F29/F30. PR #17 passed hosted CI runs [36175220393](https://github.com/kelani34/slack-vibe/actions/runs/36175220393) and [36175246491](https://github.com/kelani34/slack-vibe/actions/runs/36175246491), plus Vercel and GitGuardian; CodeRabbit review is disabled for this stacked base.
...
**Current W16 evidence:** D01 now defines measured light/dark saved, pinned, unread, success, warning, favorite and message-target roles across messaging, notifications, presence, scheduling and navigation; its contract calculates applicable text/icon/badge contrast and rejects raw app-owned palette bypasses source-wide. D02 exposes message actions to keyboard and compact touch users, inventories accessible names for every app-owned icon Button and native icon-only button, and enforces 44px compact targets through shared Button/Toggle variants; exact-artifact evidence covers the composer and formatting toolbar at 390×844. D03 gives the workspace, shared Dialog/Popover primitives, Activity, channel details and file preview dynamic viewport bounds; exact-artifact evidence covers a 320×568 pinned-message popover and PDF preview. D04 applies reduced-motion-aware loading, retry and cached-content behavior to the listed profile, thread, member, hover-card and forwarding surfaces. B20/B21 cover DM identity and successful member-card rendering; B22 rechecks the group-DM stacked-avatar/header/composer surface with shared motion tokens, and B23 verifies its Add people focus ring without layout movement. Reduced-motion browser emulation and frame evidence remain open. D05 now defines shared CSS timing/easing/travel/stagger tokens and reduced-motion overrides, consumed by avatar-stack, message-highlight and shared-button press feedback; focus-ring styling remains outside the transition properties. Most M01–M30 feature behaviors, interruption cleanup and mobile frame evidence remain open. The [design audit](design-audit.md) keeps the remaining light/forced-color, keyboard/focus, assistive-technology, tablet, device, zoom and independent-QA work open.
...
PR #12 at `140c52f` passed hosted CI runs [36162948439](https://github.com/kelani34/slack-vibe/actions/runs/36162948439) and [36162953441](https://github.com/kelani34/slack-vibe/actions/runs/36162953441), plus CodeRabbit, GitGuardian and Vercel. PR #13 at `17b964b` passed hosted CI runs [36163788447](https://github.com/kelani34/slack-vibe/actions/runs/36163788447) and [36163820211](https://github.com/kelani34/slack-vibe/actions/runs/36163820211), plus GitGuardian and Vercel. CodeRabbit reported that review was skipped for this base branch. The PR #13 documentation-sync head `2db5b55` also passed CI runs [36164185027](https://github.com/kelani34/slack-vibe/actions/runs/36164185027) and [36164190403](https://github.com/kelani34/slack-vibe/actions/runs/36164190403), with GitGuardian and Vercel green; CodeRabbit again skipped review. Independent QA and live revocation evidence remain open.

**W05 timeline-query identity guard, 25 September 2026:** `MessageList` previously enabled its paged private-history query whenever no static message list was supplied, even while `currentUserId` was absent and the cache key used the shared `anonymous` fallback. A component regression reproduced `enabled: true`; the query now waits for `currentUserId` while preserving the existing static-message behavior. The same case confirms the query is enabled when an identified viewer is present. The focused component file passes 12 tests; the full suite passes 286 tests across 61 files. Typecheck, lint, docs validation and isolated production build pass. This prevents premature client reads; server authorization remains mandatory, and no 10× latency claim is made.
