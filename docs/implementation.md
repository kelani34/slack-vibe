...
**Current W16 evidence:** D01 now defines measured light/dark saved and pinned roles for message rows, bookmark/pin affordances and editor mentions; its contract calculates every foreground/surface pair above 4.5:1 and rejects raw palette bypasses in those consumers. D02 exposes message actions to keyboard and compact touch users and labels priority profile controls. D03 uses dynamic workspace height and viewport-bounded Activity/channel-detail surfaces. The [design audit](design-audit.md) keeps the remaining color, route-wide control, overlay, device, zoom and independent-QA work open.
