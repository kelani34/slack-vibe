...
**W05 timeline-query identity guard, 25 September 2026:** `MessageList` previously enabled its paged private-history query whenever no static message list was supplied, even while `currentUserId` was absent and the cache key used the shared `anonymous` fallback. A component regression reproduced `enabled: true`; the query now waits for `currentUserId` while preserving the existing static-message behavior. The same case confirms the query is enabled when an identified viewer is present. The focused component file passes 12 tests; the full suite passes 284 tests across 61 files. Typecheck, lint, docs validation and isolated production build pass. This prevents premature client reads; server authorization remains mandatory, and no 10× latency claim is made.
