...
**W05 profile-cache update, 25 September 2026:** The profile sidebar's server read already checks access in `workspaceId`, but its React Query key previously used only the target user ID. The key now includes that workspace so a profile response authorized in one workspace cannot satisfy the same user's query in another. Red/green evidence in TC-F36-07 verifies the key shape; the profile-edit invalidation still uses the user prefix and therefore refreshes that user's entries across workspaces. Other profile/member/search/starred projections and revocation behavior remain under audit.
