...
**W09 direct upload/finalization TDD:** tests first failed for public URL returns, caller-injected attachment URLs, cross-scope/reused/expired intents, bad object size/MIME/signature, five-file/25 MB limits, and stale retry behavior. The focused PostgreSQL suites for upload, message access, and signed downloads pass 61 tests; the composer and attachment UI suites pass 15 component tests. These tests mock Supabase Storage. The full suite (309 tests/62 files) and production build pass; real bucket policy and provider behavior, authenticated browser/device transfer, interrupted-network/mobile behavior, cleanup-job races, and independent QA remain NOT RUN.
