...
**Mandatory policy:** the whole app follows [test-driven development](tdd.md). Use the [F01–F108 test matrix](test-matrix.md) before implementation. The scenarios below are implemented test-first in their owning work package; this document is not a plan to add tests after the product is built. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passed 254 local tests across the unit, component and integration projects and 57 test files; focused search/DM/avatar/message-access/context runs pass all 83 affected tests across ten files. Independent QA and the complete case catalogue remain open.

The current D04 component slices verify an accessible thread loading status; retry after initial thread query failure; cached reply retention after refresh failure; and retryable current-member and add-member query errors in both the Members tab and channel dialog. A cached member list stays visible after a failed refresh. The corresponding local browser attempt could not reach `localhost:3000` (`ERR_CONNECTION_REFUSED`); no browser evidence is claimed for these error states.
