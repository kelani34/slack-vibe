...
**W09 resumability/progress TDD, 25 September 2026:** unit tests first failed because the direct upload helper was absent; after implementation they verify endpoint/token/metadata construction, chunk/retry options, reuse only for the same bucket/path and upload-intent identity, byte progress, interrupted transfer and exhausted-error handling. Integration tests first failed because grant renewal was not exposed, then verify same-owner/same-workspace/same-channel/same-pending-intent renewal and denial for anonymous, wrong actor, workspace-removed actors with stale channel rows, expired, completed or malformed intents. Hook and component tests cover same-intent resume and accessible composer progress. The four focused files pass 26 tests; the full suite/build/docs contract, typecheck and lint pass. Storage is mocked. Hosted verification, real-provider/mobile interruption testing, explicit user pause/cancel controls and independent QA remain NOT RUN. See [TDD acceptance F29](acceptance-cases.md#f29-attachment-upload).
