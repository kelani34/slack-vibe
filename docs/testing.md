...
**Mandatory policy:** the whole app follows [test-driven development](tdd.md). Use the [F01–F108 test matrix](test-matrix.md) before implementation. The scenarios below are implemented test-first in their owning work package; this document is not a plan to add tests after the product is built. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passed 309 local tests across the unit, component and integration projects and 62 test files. The W09-focused run passed 61 integration tests and 15 component tests. Independent QA and the complete case catalogue remain open. One intervening full-suite attempt timed out in `src/components/direct-message-actions.test.tsx` while renaming a group; the focused file then passed all three tests and a subsequent full run passed all 274 tests. Keep this as a transient flakiness observation until repeated hosted runs remain green.
...
**W09 media-signature follow-up:** the integration test first failed because arbitrary bytes labeled `video/mp4` reached storage. The action now rejects the mismatch before storage access and accepts a matching MP4 file-type box. The focused upload file passes 8 tests; the current full suite passes 289 tests across 61 files. Hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), plus Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. These are prefix checks; full decoding, scanning, private storage and signed delivery remain open.

**W09 authorized-read foundation:** PostgreSQL integration tests cover the object-locator pair constraint, anonymous/outsider/deleted/future denial, channel-prefixed object paths, preview/download grant parameters and provider failure. Component tests cover a private file using signed URLs instead of its legacy URL, no private thumbnail request, visible focus/loading state, denial retry, and legacy URL compatibility. The provider is mocked; actual private bucket, expiry, revocation, browser/device and independent-QA checks remain open. PR #17 passed hosted CI runs [36175220393](https://github.com/kelani34/slack-vibe/actions/runs/36175220393) and [36175246491](https://github.com/kelani34/slack-vibe/actions/runs/36175246491), Vercel and GitGuardian; CodeRabbit review is disabled for this stacked base.

**W09 direct upload/finalization TDD:** tests first failed for public URL returns, caller-injected attachment URLs, cross-scope/reused/expired intents, bad object size/MIME/signature, five-file/25 MB limits, and stale retry behavior. The focused PostgreSQL suites for upload, message access, and signed downloads pass 61 tests; the composer and attachment UI suites pass 15 component tests. These tests mock Supabase Storage. The full suite (309 tests/62 files) and production build pass. PR #18 at `7d6434b` passed hosted runs [36180833968](https://github.com/kelani34/slack-vibe/actions/runs/36180833968) and [36180873382](https://github.com/kelani34/slack-vibe/actions/runs/36180873382), plus Vercel and GitGuardian; CodeRabbit skipped review for the stacked base. Real bucket policy/provider behavior, authenticated browser/device transfer, interrupted-network/mobile behavior, cleanup-job races, and independent QA remain NOT RUN.

**W09 resumability/pause/cancel TDD, 25 September 2026:** unit tests first failed because the direct upload helper was absent; they now verify endpoint/token/metadata construction, chunk/retry options, same-intent resume, byte progress, interrupted transfer and exhausted-error handling. Integration tests verify same-owner/workspace/channel intent renewal and denial for anonymous, wrong actor, workspace-removed actors with stale channel rows, expired, completed or malformed intents. Hook/component tests cover pause removing the optimistic row without a false failed-message state, preserving the same file/intent for retry, cancel/remove during upload, accessible progress and distinct paused/completed status. The focused four-file run passes 28 tests; the full suite passes 322 tests across 64 files, and typecheck, lint, docs contract and production build pass. PR #19 head `8735534` passed hosted CI runs [36225657406](https://github.com/kelani34/slack-vibe/actions/runs/36225657406) and [36225659673](https://github.com/kelani34/slack-vibe/actions/runs/36225659673), GitGuardian and Vercel; CodeRabbit skipped review for the stacked base. Supabase is mocked. Provider-backed behavior, authenticated browser/device/mobile interruption, cleanup, full decoding/scanning and independent QA remain NOT RUN. The deployed preview requires Vercel authentication and the open localhost app shows a client fetch error. See [TDD acceptance F29](acceptance-cases.md#f29-attachment-upload).

**W09 bounded orphan cleanup, 26 September 2026:** `src/app/api/cron/cleanup-uploads/route.test.ts` covers authenticated invocation, unauthorized and missing-secret behavior, no-work short-circuit, selection bounds/grace/reference filters, deletion order, and retryable storage/database errors. `tests/integration/upload-cleanup-index.test.ts` verifies the additive expiry index against a fresh migrated PostgreSQL database. Local full-suite validation passes 328 tests across 66 files; typecheck, full lint, production build, docs contract and diff checks pass. PR #19 head `602d528` passed hosted CI runs [36234589711](https://github.com/kelani34/slack-vibe/actions/runs/36234589711) and [36234592350](https://github.com/kelani34/slack-vibe/actions/runs/36234592350), GitGuardian and Vercel. The live private bucket/provider, Vercel secret/invocation and independent QA remain NOT RUN.
