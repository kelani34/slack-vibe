...
**Mandatory policy:** the whole app follows [test-driven development](tdd.md). Use the [F01–F108 test matrix](test-matrix.md) before implementation. The scenarios below are implemented test-first in their owning work package; this document is not a plan to add tests after the product is built. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passed 241 local tests across the unit, component and integration projects and 56 test files; focused search/DM/avatar/message-access/context runs pass all 83 affected tests across ten files. Independent QA and the complete case catalogue remain open.
