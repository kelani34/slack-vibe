...
**W09 first verification slice:** upload integration cases now reject SVG and raster-image signature mismatches before storage access. The focused file passes 6 tests; the latest full suite is 287 tests across 61 files. Permanent public URLs, the browser avatar path, complete content validation, signed access, attachment finalization and bounded transfer remain the next W09 work.
