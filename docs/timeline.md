...
The current D04 slice aligns profile, thread, and channel-member loading: each announces the surface being loaded, reserves representative content geometry, and honors reduced motion through shared skeleton behavior. Profile, thread, current-member, add-member, forwarding destination and member-hover-card queries now have accessible retry states, while cached content remains visible/usable during failed background refreshes. Forwarding announces loading and the empty destination state. Hover-card profile reads wait until the card opens, reducing unnecessary requests for rendered message authors. The follow-on D05 motion foundation defines shared CSS timing/easing/travel/stagger tokens, routes the group-DM avatar stack and message-target highlight through them, and zeros duration, delay and travel under reduced motion; the token contract is test-covered. Focused component/contract tests passed after demonstrating the intended red results. Browser inspection of motion/reduced-motion states, cross-route empty/progress/denied/archived/revoked states, broader error coverage and independent QA remain open.

Implementation began after the local GitHub OAuth path was repaired. The latest complete suite passes 300 automated tests across 62 files in unit, component and integration projects; focused search/DM/avatar/message-access runs pass 83 tests across 10 files, and current typecheck, production build and full-repository lint pass. PR #6 hosted CI runs [36149667142](https://github.com/kelani34/slack-vibe/actions/runs/36149667142) and [36149672117](https://github.com/kelani34/slack-vibe/actions/runs/36149672117) passed. PR #7 at `d777da4a1ead74fe9e214348d9eb99fe90c0cddc` passed CI runs [36151023880](https://github.com/kelani34/slack-vibe/actions/runs/36151023880) and [36151029811](https://github.com/kelani34/slack-vibe/actions/runs/36151029811). PR #8 at `ebdd2e96e6488b3b05e11c4192b10b0b5dec5945` passed latest CI runs [36151946486](https://github.com/kelani34/slack-vibe/actions/runs/36151946486) and [36151973789](https://github.com/kelani34/slack-vibe/actions/runs/36151973789), plus GitGuardian and Vercel checks. PR #9 at `990f76f` passed hosted CI runs [36154871519](https://github.com/kelani34/slack-vibe/actions/runs/36154871519) and [36154907966](https://github.com/kelani34/slack-vibe/actions/runs/36154907966), plus GitGuardian and Vercel checks. PR #10 at `09aa5ca` passed hosted CI runs [36156507989](https://github.com/kelani34/slack-vibe/actions/runs/36156507989) and [36156547919](https://github.com/kelani34/slack-vibe/actions/runs/36156547919), plus GitGuardian and Vercel checks. PR #11 at `23ca9d4` passed hosted CI runs [36158693946](https://github.com/kelani34/slack-vibe/actions/runs/36158693946) and [36158731887](https://github.com/kelani34/slack-vibe/actions/runs/36158731887), plus CodeRabbit, GitGuardian and Vercel checks. These checks do not constitute independent QA. Deliberate CI failure propagation remains open. The D01–D04 slices add measured light/dark semantic roles across messaging, notifications, presence, scheduling and navigation, test-first keyboard/touch message actions, source-wide icon-control names, shared 44px compact Button/Toggle targets, dynamic workspace height, and viewport-bounded shared Dialog/Popover, Activity, channel-details and file-preview surfaces. D05 centralizes motion timing/easing/travel/stagger, applies the tokens to the avatar stack/message highlight, and adds shared button press feedback that keeps focus styling immediate and removes scale under reduced motion. Browser review of press timing/reduced motion and M01–M30 adoption remains open. The W04 interactive send path now persists a stable actor-scoped intent key, deduplicates concurrent/retried requests and reuses completed attachment uploads during an in-page retry. W39 group creation also persists a stable intent key and private request hash, so lost-response, concurrent and page-reload retries resolve to one group while changed-payload and cross-creator reuse are rejected. The candidate also includes documented workspace/channel/message/profile/search/file-upload/direct-message authorization, allowlist-based message HTML sanitization at write and read boundaries, safe plain-text system-event writes, an access-checked member directory with one-click DM entry, a dedicated Direct messages sidebar, a pair-keyed one-to-one model, a responsive DM inbox with database-bounded activity pages, participant/latest-message/unread summaries and server-side unread filtering, one-to-one/group recipient creation, new-generation group expansion, group rename/leave actions after membership shrinks, stable stacked participant avatars across the sidebar, inbox and conversation header (including groups with two active members and the one-active-member group-marker fallback), mobile Back navigation, per-user composer draft restore/clear behavior and logout purge; same-tab sign-out and authenticated actor changes now clear React Query state; member-profile cache keys now include workspace scope and member-management lists are actor/workspace/channel-scoped, with component TDD; participant-aware conversation header and composer placeholder, forwarding, scheduled edit/send-now/cancellation, invite-link copy, bookmark privacy filtering, access-filtered notification paging and relational mark-read, focused-conversation-only message hydration, published-reply invalidation for thread and root timeline caches, preference and invite flows without broad route invalidation, Next.js 16 proxy/login matcher regressions, validated post-login return paths, safe auth-error messaging and a functional account-menu sign-out control. Search now has query/actor-bound keyset pagination, a tested GIN substring path, bounded filter parsing, visible invalid-input feedback, explicit date semantics, bounded old-message context navigation, reply-to-thread routing, restoration to the latest timeline, retryable context failures and component-tested history/viewport restoration. The [DM contract](direct-messages.md) records the implemented F13/F52 creation and inbox slices and the remaining group lifecycle, privacy, media, offline, mobile and governance work. Local browser evidence B07–B09 verifies requested-route return, logout and protected-route denial; B11 verifies only basic route-level Back/Forward, while B18 verifies the exact production composer/formatting controls at 390×844 and B19 verifies a pinned-message popover/PDF preview at 320×568, B23 verifies a visible keyboard focus ring on the group-DM Add people action without layout movement, and B24 verifies logout/re-authentication in the authenticated local workspace after Query-cache clearing was added. Deliberate hosted workflow failure propagation, deployed callback, expiry/error recovery, broader cold/filtered/concurrent and production-region search performance, live search/saved/notification Back/Forward, focus/scroll/mobile journeys and independent full-feature QA remain open gates. Five opt-in runs on the fixed 100k-message local search fixture recorded selective warm p50 12.08–52.47ms and p95 13.53–121.68ms; see [performance evidence](performance.md).

The next sequence is dependency-first:

1. **Close P00/P01:** local tests/typecheck/build/lint are green. PR #6 hosted CI runs [36149667142](https://github.com/kelani34/slack-vibe/actions/runs/36149667142) and [36149672117](https://github.com/kelani34/slack-vibe/actions/runs/36149672117) passed. PR #7 at `d777da4a1ead74fe9e214348d9eb99fe90c0cddc` passed the latest checks [36151023880](https://github.com/kelani34/slack-vibe/actions/runs/36151023880) and [36151029811](https://github.com/kelani34/slack-vibe/actions/runs/36151029811). Deliberately failing a test to prove CI blocks remains open. B07–B09 cover local sign-in return/logout/protected-route denial; B10 shows the named OAuth app has a local callback, but deployment-to-client-ID mapping is unknown. Confirm that mapping, then verify deployed callback, expiry/error paths, realtime identity and independent two-actor access.
2. **Continue W05/P02 speed work:** channel unread counts, DM activity pages, notification pagination, channel-notification read updates and search page transfer have bounded PostgreSQL paths and TDD coverage. Sidebar and destination-list projections are narrowed; workspace membership prevents stale channel rows from surviving revocation; the sidebar owns unread state and the focused view owns message hydration. Schedule creation/edit/cancel/send-now and channel/DM lifecycle mutations use owning client views and have regressions against full-route invalidation; preferences and invite acceptance no longer refresh broad route trees. Profile, member-list, timeline, parent-message and thread cache keys now carry the relevant actor/workspace/resource identity; optimistic send/retry and realtime catch-up use the same timeline/thread keys. Red/green tests cover profile-card outsider denial and viewer/workspace/target cache scope (PR #11), scheduled-message cache scope and actor-gated query enablement, scheduled-row denial after stale channel membership/workspace removal, narrow scheduled projections, and parent/reply query disablement until thread viewer identity is known; both member-list surfaces, timeline/thread reads, reconnect invalidation, optimistic send/retry, scoped message actions, channel-bound bookmark loading and saved-row denial after workspace removal. Remaining audit: verify other cache contracts and live permission revocation; the status-based reconnect catch-up has component coverage, while real network-loss and multi-tab convergence still need browser evidence; trace initial message hydration and mounted-shell bursts; measure cold/warm production-build latency, production-region behavior and the remaining search/load profiles. Five opt-in 100k-message search runs cover six query shapes, an eight-request burst and natural/forced plans; natural GIN selection varied, so this does not establish a speedup. B11 verifies only basic route-level Back/Forward; deep-link query, viewport/focus restoration and mobile behavior remain next. Private-object delivery and draft sync remain open.
...
**Current dependency checkpoint, 26 September 2026:** W09 has private upload intents, direct browser-to-storage transfer, object verification, resumable TUS, progress, pause, cancel/remove, same-intent renewal and retry. Commits `54fdf1d`, `8735534`, `57c5de9`, `8d24f96` and `602d528` are pushed on `feature/resumable-attachment-uploads`; PR #19 head `602d528` passed CI runs [36234589711](https://github.com/kelani34/slack-vibe/actions/runs/36234589711) and [36234592350](https://github.com/kelani34/slack-vibe/actions/runs/36234592350), GitGuardian and Vercel; CodeRabbit skipped review for the stacked base. The bounded daily cleanup route, one-hour expiry grace, attachment-reference filter and expiry index pass local route/PostgreSQL tests and hosted checks. The private bucket/provider and CRON_SECRET, full content validation, avatar migration, authenticated browser/device/mobile network verification and independent QA remain open. The Vercel preview still requires Vercel authentication, and the open localhost app has a client-side fetch error. W09 remains in progress and ahead of W10; do not advance the feature based on mocked-provider tests alone.

PR #18 at `7d6434b` passed hosted test/type/build/lint runs [36180833968](https://github.com/kelani34/slack-vibe/actions/runs/36180833968) and [36180873382](https://github.com/kelani34/slack-vibe/actions/runs/36180873382), Vercel and GitGuardian. CodeRabbit skipped review for the stacked base. PR #19 pause/cancel and hosted-documentation follow-ups passed runs [36225657406](https://github.com/kelani34/slack-vibe/actions/runs/36225657406) and [36225659673](https://github.com/kelani34/slack-vibe/actions/runs/36225659673), then [36229370864](https://github.com/kelani34/slack-vibe/actions/runs/36229370864) and [36229373174](https://github.com/kelani34/slack-vibe/actions/runs/36229373174). The cleanup follow-up on `602d528` passed runs [36234589711](https://github.com/kelani34/slack-vibe/actions/runs/36234589711) and [36234592350](https://github.com/kelani34/slack-vibe/actions/runs/36234592350), with GitGuardian and Vercel green across these pushed checkpoints. CodeRabbit skipped review because the PR targets a stacked base. The preview requires Vercel authentication; authenticated upload-flow browser/device evidence remains open.

**W09 pause/cancel implementation, 25 September 2026:** a user can pause the active transfer while keeping the composer draft/file and intent, then press Send to resume; removing the active attachment cancels the in-flight send before publication and removes only that file. Aborted sends clear the temporary optimistic row instead of leaving a false failed message. Expired intents get a new scoped intent. Focused tests pass 28 cases; the full suite passes 322 tests across 64 files, with typecheck, lint, docs contract and production build green. Commits `54fdf1d` and `8735534` are pushed, and PR #19's current head has passing hosted checks recorded above. Provider bucket, partial-object cleanup, authenticated browser/mobile interruption and independent QA remain open.
...
PR #12 at `140c52f` passed CI runs [36162948439](https://github.com/kelani34/slack-vibe/actions/runs/36162948439) and [36162953441](https://github.com/kelani34/slack-vibe/actions/runs/36162953441), plus CodeRabbit, GitGuardian and Vercel. PR #13 at `17b964b` passed hosted CI runs [36163788447](https://github.com/kelani34/slack-vibe/actions/runs/36163788447) and [36163820211](https://github.com/kelani34/slack-vibe/actions/runs/36163820211), plus GitGuardian and Vercel. CodeRabbit reported that review was skipped for this base branch. The PR #13 documentation-sync head `2db5b55` also passed CI runs [36164185027](https://github.com/kelani34/slack-vibe/actions/runs/36164185027) and [36164190403](https://github.com/kelani34/slack-vibe/actions/runs/36164190403), with GitGuardian and Vercel green; CodeRabbit again skipped review. Independent QA remains open.

**W05 next cache guard:** After the thread-sidebar queries, the paged root timeline query also waits for the current viewer identity before fetching private history. The failing component regression now passes in both anonymous and identified cases. The full local suite is at 286 tests across 61 files; typecheck, lint, docs check and isolated build pass. Hosted verification and independent QA for this slice remain open.
...
**W09 first verification slice:** upload integration cases now reject SVG and raster-image signature mismatches before storage access. The focused file passes 6 tests; the latest full suite is 287 tests across 61 files. Hosted CI runs [36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472) and [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785), Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. Permanent public URLs, the browser avatar path, complete content validation, signed access, attachment finalization and bounded transfer remain the next W09 work.

**W09 media-signature follow-up:** red/green integration tests now reject arbitrary bytes declared as `video/mp4` and accept a matching MP4 file-type box. The upload action checks supported image/video/audio/PDF prefixes within 12 bytes. The focused integration file passes 8 tests; the full local suite passes 289 tests across 61 files. Hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), plus Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. This still does not validate complete container structure, Office formats, full decoding, scanning or private delivery.

**W09 authorized private-read foundation:** the attachment schema now supports an optional bucket/path locator with a database pair constraint and uniqueness. Current channel membership, visible-message state and channel-prefixed path are checked before issuing separate 5-minute preview/download URLs. Message cards request grants on open, expose focus/loading/retry states and never use the legacy URL as a private image thumbnail; URL-only legacy attachments remain compatible. TDD added eight integration cases and three component cases since the last full checkpoint; the full suite now passes 300 tests across 62 files, and typecheck, lint, docs validation and isolated production build pass locally. The bucket is not provisioned, existing uploads remain public, grant expiry/revocation and independent QA are open. PR #17 passed hosted CI runs [36175220393](https://github.com/kelani34/slack-vibe/actions/runs/36175220393) and [36175246491](https://github.com/kelani34/slack-vibe/actions/runs/36175246491), Vercel and GitGuardian; CodeRabbit review is disabled for this stacked base.

**Earlier W09 TUS checkpoint, 25 September 2026:** red/green coverage proved that browser uploads use the direct Supabase Storage TUS endpoint with the scoped signature token, 6 MB chunks, intent-bound fingerprints and automatic retry; resumed uploads renew the grant for the same pending intent only after actor, workspace and channel membership checks. The composer exposed accessible byte progress, and message retry passed the same upload-intent IDs. At that checkpoint, focused suites passed 26 tests across four files and the full suite passed 320 tests across 64 files. PR #19 code head `fa5b97e` passed hosted runs [36187621261](https://github.com/kelani34/slack-vibe/actions/runs/36187621261) and [36189625943](https://github.com/kelani34/slack-vibe/actions/runs/36189625943), GitGuardian and Vercel; CodeRabbit review was skipped for this stacked base. Explicit pause/cancel UI was still open at that point; this is now implemented with developer tests, while provider/mobile execution, cleanup, avatar migration and independent QA remain open. The registered branch is `feature/resumable-attachment-uploads`, with no `codex` substring. See [TUS protocol design](system-design.md#file-lifecycle), [performance validation](performance.md#observed-opportunities-ranked-by-return), and [acceptance cases](acceptance-cases.md#f29-attachment-upload).
