...
**W09 TUS progress/resume follow-up, 25 September 2026:** red/green coverage now proves that browser uploads use the direct Supabase Storage TUS endpoint with the scoped signature token, 6 MB chunks, intent-bound fingerprints and automatic retry; resumed uploads renew the grant for the same pending intent only after actor, workspace and channel membership checks. The composer exposes accessible byte progress, and message retry passes the same upload-intent IDs. The focused suites pass 26 tests across four files; full suite (320 tests/64 files), typecheck, lint, docs contract and production build pass locally. Hosted CI for this head is still pending. Explicit pause/cancel UI, live provider/bucket, interruption testing on physical mobile devices, cleanup, avatar migration and independent QA remain open. The registered branch is `feature/resumable-attachment-uploads`, with no `codex` substring. See [TUS protocol design](system-design.md#file-lifecycle), [performance validation](performance.md#observed-opportunities-ranked-by-return), and [acceptance cases](acceptance-cases.md#f29-attachment-upload).
