...
Assessed **18 September 2026**, against commit **13a87c4**. This documentation remains the scope and implementation authority; the active follow-up has since added verified core DM and all-unread inbox slices, pageable indexed search, bounded old-message context jumps and a repeatable local search benchmark. The latest full run passes 309 TDD tests across 62 files. Actor/workspace-scoped message, saved-item and member-list caches now share keys through reads, optimistic sends, realtime catch-up and action invalidation. Profile and hover-card caches include viewer/workspace/target scope, and hover-card reads require current requester and target membership. Scheduled-message reads are scoped by actor/workspace/channel/thread, require current workspace and channel membership, and return a bounded field projection. The conversation pin/bookmark panel fetches only the current channel’s bookmarks. Five 100k-message local search runs cover short/common terms, filters and concurrent requests; selective warm p50 ranges from 12.08–52.47ms and the natural planner chooses the trigram index in two of five runs. This remains synthetic local evidence, not a production or whole-app 10× claim.
...
**The whole application must be built with TDD.** [Test-first development](tdd.md) defines red → green → refactor, test layers, coverage and CI gates. The [108-requirement test matrix](test-matrix.md) assigns behavior/failure coverage to every F requirement. Tests are part of each implementation slice, not a final QA phase. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passes 309 unit, component and integration tests across 62 files; the full catalogue and independent QA remain open.

The first GitHub Actions workflow is configured in `.github/workflows/ci.yml`. PR #6 passed runs [36149667142](https://github.com/kelani34/slack-vibe/actions/runs/36149667142) and [36149672117](https://github.com/kelani34/slack-vibe/actions/runs/36149672117); PR #7 at `d777da4a1ead74fe9e214348d9eb99fe90c0cddc` passed [36151023880](https://github.com/kelani34/slack-vibe/actions/runs/36151023880) and [36151029811](https://github.com/kelani34/slack-vibe/actions/runs/36151029811); PR #8 at `ebdd2e96e6488b3b05e11c4192b10b0b5dec5945` passed [36151946486](https://github.com/kelani34/slack-vibe/actions/runs/36151946486) and [36151973789](https://github.com/kelani34/slack-vibe/actions/runs/36151973789), plus GitGuardian and Vercel checks. The message-cache slice in [PR #9](https://github.com/kelani34/slack-vibe/pull/9) at commit `990f76f` passes local tests, typecheck, lint, docs checks and production build; it passed hosted CI runs [36154871519](https://github.com/kelani34/slack-vibe/actions/runs/36154871519) and [36154907966](https://github.com/kelani34/slack-vibe/actions/runs/36154907966), plus GitGuardian and Vercel checks. Red/green component coverage verifies actor/workspace/resource query keys and workspace-bounded invalidation after pinning. The saved-message cache slice in [PR #10](https://github.com/kelani34/slack-vibe/pull/10) at `09aa5ca` passes local TDD, typecheck, lint, docs and build checks; PR #10 at `09aa5ca` passed hosted CI runs [36156507989](https://github.com/kelani34/slack-vibe/actions/runs/36156507989) and [36156547919](https://github.com/kelani34/slack-vibe/actions/runs/36156547919), plus GitGuardian and Vercel checks. PR #11 at `23ca9d4` passes hosted CI runs [36158693946](https://github.com/kelani34/slack-vibe/actions/runs/36158693946) and [36158731887](https://github.com/kelani34/slack-vibe/actions/runs/36158731887), CodeRabbit, GitGuardian and Vercel checks. These are developer checks, not independent feature QA. Local lint began at 84 errors and 32 warnings and has now been reduced to zero errors and zero warnings. Search’s opt-in performance benchmark is separate from that regular suite; independent QA and later release gates remain open.
...
PR #12 at `140c52f` passed CI runs [36162948439](https://github.com/kelani34/slack-vibe/actions/runs/36162948439) and [36162953441](https://github.com/kelani34/slack-vibe/actions/runs/36162953441), with CodeRabbit, GitGuardian and Vercel green. PR #13 at `17b964b` passed hosted CI runs [36163788447](https://github.com/kelani34/slack-vibe/actions/runs/36163788447) and [36163820211](https://github.com/kelani34/slack-vibe/actions/runs/36163820211), plus GitGuardian and Vercel. CodeRabbit reported that review was skipped for this base branch. The PR #13 documentation-sync head `2db5b55` also passed CI runs [36164185027](https://github.com/kelani34/slack-vibe/actions/runs/36164185027) and [36164190403](https://github.com/kelani34/slack-vibe/actions/runs/36164190403), with GitGuardian and Vercel green; CodeRabbit again skipped review. The thread-query viewer guard has focused and full local test evidence; independent QA remains open.

**Latest W05 local slice:** The paged message timeline query now waits for the viewer identity, and a component regression confirms it stays disabled without an actor and enabled when the actor is known. The test was observed failing before the guard; the focused file passes 12 tests and the full local suite passes 287 across 61 files. Independent QA and hosted PR verification for this slice remain open.
...
**W09 security checkpoint:** the upload action rejects SVG and checks declared image, video, audio and PDF prefixes before storage. Red/green integration coverage now includes mismatched and matching PNG/MP4 headers; this remains prefix validation, not full decoding or scanning. Permanent public URLs, the browser avatar uploader, complete document validation, signed downloads, and bounded direct transfer remain open. The latest full local suite passes 289 tests across 61 files. W09 media-signature PR #16 hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), plus Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. W09 upload-signature PR #15 passed hosted CI runs [36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472) and [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785), plus Vercel and GitGuardian; its CodeRabbit review was disabled for the stacked base. At that earlier W09 checkpoint, no upload wrote private references. The following direct-transfer slice now changes that path; see current evidence below.

**Current W09 private-file progress:** attachment records can carry an optional bucket/path pair, and the message UI requests access-checked 5-minute preview/download URLs only on open. Upload creates a 2-hour workspace/channel-bound intent and path-scoped token; browser bytes go directly to storage, then finalization verifies object path, size/MIME and a short media signature prefix. Message creation accepts only unique, unexpired finalized IDs and database-sourced metadata, with five-file/25 MB aggregate limits. TUS uses 6 MB chunks, same-intent resume/retry, accessible progress, Pause and Cancel upload/Remove attachment. A daily CRON_SECRET-protected route now removes at most 100 expired unattached objects after a one-hour grace period, with provider deletion before metadata deletion and an expiry index for the scan. Route and expiry-index tests pass, and the full suite passes 328 tests across 66 files with typecheck, lint, docs contract and production build green locally. Commits `8d24f96` and `602d528` are pushed on `feature/resumable-attachment-uploads`; PR #19 head `602d528` passed CI runs [36234589711](https://github.com/kelani34/slack-vibe/actions/runs/36234589711) and [36234592350](https://github.com/kelani34/slack-vibe/actions/runs/36234592350), GitGuardian and Vercel. The private bucket/provider, CRON_SECRET deployment and live provider cleanup remain unverified. Avatar migration, full content scanning, authenticated browser/device, mobile interruption and independent QA remain open. The Vercel preview requires Vercel authentication, and the open localhost app displays a client-side fetch error; neither proves authenticated upload behavior.
