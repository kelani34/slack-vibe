...
**The whole application must be built with TDD.** [Test-first development](tdd.md) defines red → green → refactor, test layers, coverage and CI gates. The [108-requirement test matrix](test-matrix.md) assigns behavior/failure coverage to every F requirement. Tests are part of each implementation slice, not a final QA phase. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passes 300 unit, component and integration tests across 62 files; the full catalogue and independent QA remain open.
...
**W09 security checkpoint:** the upload action rejects SVG and checks declared image, video, audio and PDF prefixes before storage. Red/green integration coverage now includes mismatched and matching PNG/MP4 headers; this remains prefix validation, not full decoding or scanning. Permanent public URLs, the browser avatar uploader, complete document validation, signed downloads, and bounded direct transfer remain open. The latest full local suite passes 289 tests across 61 files. W09 media-signature PR #16 hosted CI runs [36167150692](https://github.com/kelani34/slack-vibe/actions/runs/36167150692) and [36167158039](https://github.com/kelani34/slack-vibe/actions/runs/36167158039), plus Vercel and GitGuardian passed; CodeRabbit review was disabled for the stacked base. W09 upload-signature PR #15 passed hosted CI runs [36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472) and [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785), plus Vercel and GitGuardian; its CodeRabbit review was disabled for the stacked base. A new W09 slice adds optional object locators and an access-checked 5-minute preview/download path, but no current upload writes private references yet; full validation remains pending.

**Current W09 private-read progress:** attachment records can carry an optional bucket/path pair, and the message UI now requests access-checked 5-minute preview/download URLs only on open. Deleted/future messages, anonymous/outsider callers, mismatched channel paths and grant failures are covered in PostgreSQL integration tests; component tests cover private URL use, loading, retry and legacy compatibility. Current full-suite validation is 300 tests across 62 files; typecheck, lint, docs validation and isolated production build pass locally. Actual bucket configuration and provider/device/independent QA are not verified. PR #17 passed hosted CI runs [36175220393](https://github.com/kelani34/slack-vibe/actions/runs/36175220393) and [36175246491](https://github.com/kelani34/slack-vibe/actions/runs/36175246491), Vercel, and GitGuardian; CodeRabbit review is disabled for this stacked base.
