...
**The whole application must be built with TDD.** [Test-first development](tdd.md) defines red → green → refactor, test layers, coverage and CI gates. The [108-requirement test matrix](test-matrix.md) assigns behavior/failure coverage to every F requirement. Tests are part of each implementation slice, not a final QA phase. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passes 287 unit, component and integration tests across 61 files; the full catalogue and independent QA remain open.
...
**W09 security checkpoint:** the upload action now rejects SVG and mismatched raster-image signatures before storage. This closes only the tested image-prefix cases; permanent public URLs, the browser avatar uploader, complete file validation, signed downloads, and bounded direct transfer remain open. The latest full local suite passes 287 tests across 61 files. W09 upload-signature PR #15 passed hosted CI runs [36166337472](https://github.com/kelani34/slack-vibe/actions/runs/36166337472) and [36166359785](https://github.com/kelani34/slack-vibe/actions/runs/36166359785), plus Vercel and GitGuardian; its CodeRabbit review was disabled for the stacked base.
