...
**The whole application must be built with TDD.** [Test-first development](tdd.md) defines red → green → refactor, test layers, coverage and CI gates. The [108-requirement test matrix](test-matrix.md) assigns behavior/failure coverage to every F requirement. Tests are part of each implementation slice, not a final QA phase. The latest full run, `npm test -- --maxWorkers=1 --testTimeout=15000`, passes 228 unit, component and integration tests across 53 files; the full catalogue and independent QA remain open.

The first GitHub Actions workflow is configured in `.github/workflows/ci.yml`; [run 36124016907](https://github.com/kelani34/slack-vibe/actions/runs/36124016907) passed on current pushed commit `52345d1`. Local lint began at 84 errors and 32 warnings and has now been reduced to zero errors and zero warnings. Current lint, typecheck, production build and the 228-test full suite pass. Search’s opt-in performance benchmark is separate from that regular suite; independent QA and later release gates remain open.
