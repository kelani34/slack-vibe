...
**Current W09 private-file progress:** attachment records can carry an optional bucket/path pair, and the message UI requests access-checked 5-minute preview/download URLs only on open. Upload creates a 2-hour workspace/channel-bound intent and path-scoped token; browser bytes go directly to storage, then finalization verifies object path, size/MIME and a short media signature prefix. Message creation accepts only unique, unexpired finalized IDs and database-sourced metadata, with five-file/25 MB aggregate limits. The current follow-up uses TUS with 6 MB chunks, same-intent resume/retry and accessible byte progress; four focused test files pass 26 cases, and the full suite (320 tests after the stale-workspace-member regression), typecheck, lint, docs and production build pass locally. PR #19 code head `fa5b97e` passed hosted tests/types/build/lint, GitGuardian and Vercel; CodeRabbit review was skipped for this stacked base. Provider behavior is mocked and the private bucket is not provisioned or verified. Explicit pause/cancel controls, orphan cleanup, avatar migration, full content scanning, authenticated browser/device and independent QA remain open. The separate worktree browser preview redirected repeatedly and is not evidence of this branch's authenticated upload flow. PR #19 is linked from the implementation and timeline records.
