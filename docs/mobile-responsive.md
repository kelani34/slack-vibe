...
| [File preview](../src/components/file-preview-modal.tsx) | Original `90vh` viewer now uses 1rem `dvh`/`vw` margins and retained close/download controls; private cards are named buttons with visible focus, loading and retry states | Source repaired and measured at 288×536 inside a 320×568 viewport; component tests cover private grant use; test short landscape, signed-media expiry, loading/failure and physical devices | W09/W16 |
...
Current D02 source evidence enforces 44px compact targets for shared icon Button and Toggle variants and accessible names for app-owned icon controls. At 390×844, the exact production artifact measures the composer controls and all eight expanded formatting controls at 44×44px and wraps the toolbar without horizontal clipping. This is one developer viewport check; 320px, enlarged text/zoom, keyboard-open layouts, assistive technology, physical devices and independent QA remain required.
