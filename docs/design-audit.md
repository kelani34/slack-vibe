...
4. **D04, state language — in progress:** profile, thread, and channel-member loading use accessible, surface-shaped skeletons with reduced-motion behavior; profile request errors have a retry path. Cross-route empty/progress/error/denied/archived/revoked consistency remains open.
