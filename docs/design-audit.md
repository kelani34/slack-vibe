...
- **Implemented slice, 25 September 2026:** Profile, thread reply, and channel-member loading now use content-shaped skeletons inside named `role="status"` regions. Shared skeletons pulse only when motion is allowed. Profile, thread, current-member, and add-member query failures now use alerts with explicit retry actions; stale profile/reply/member content stays visible when a background refresh fails. Component tests assert accessible names, content shapes, reduced-motion behavior, retry, and retained content.
...
4. **D04, state language — in progress:** profile, thread, and channel-member loading use accessible, surface-shaped skeletons with reduced-motion behavior; profile, thread, and both channel-member queries have retry paths, preserving cached content during failed refreshes. Cross-route empty/progress/error/denied/archived/revoked consistency remains open.
