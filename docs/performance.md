...
| 7 | Sequential upload via server buffer | Signed direct TUS upload with 6 MB chunks, intent-scoped resume fingerprints, retry and byte progress | Avoids server buffering and restarts after transient interruption; total duration remains network-bound and has not been measured | 1/3/5-file mobile/network trace, retry duration, browser memory and provider telemetry |
...
The W05 client cache audit also gates private root-history and thread queries until actor identity is known, avoiding requests under fallback identities. This reduces premature query work but has not been measured as a latency improvement; record request/query counts and cold/warm timing before making a speed claim. W09 sends upload bytes directly from the browser to private Supabase Storage using a two-hour path-scoped token, avoiding the Next Server Action default 1 MB request cap and Vercel Function 4.5 MB body cap. Finalization reads object metadata and requests only bytes 0–11 for supported media signatures; this bounds verification transfer but has not been benchmarked under mobile network conditions. A TUS client now reports byte progress, uploads 6 MB chunks, retries transient failures and resumes an interrupted transfer under the same upload-intent fingerprint after the server renews that intent's grant. Composer controls can pause while keeping the draft/file and cancel by removing a file while stopping the in-flight send. A daily bounded cleanup scans the expiry index and removes only unattached objects after a one-hour grace period; it is operationally asynchronous and does not affect upload latency. These improve recovery and visibility; they do not promise higher throughput. Benchmark completion time and memory over 1/3/5-file cases on slow and interrupted mobile networks before claiming a speedup. Opening a path-bearing attachment still requests two signed URLs (inline preview plus forced download), valid for five minutes; no signing latency or batched visible-window projection has been measured. Do not persist signed URLs or share them across viewer scopes.
