...
The W05 client cache audit also gates private root-history and thread queries until actor identity is known, avoiding requests under fallback identities. This reduces premature query work but has not been measured as a latency improvement; record request/query counts and cold/warm timing before making a speed claim. W09 image-signature checks read at most 12 bytes before upload, but the current server upload still buffers the complete file; direct transfer and 1/3/5-file memory traces remain required.
