...
| ID | Capability / current source coverage | Remaining acceptance criteria | Day one | Package |
|---|---|---|---|---|
| F29 | Attachment upload: Partial; direct signed upload token, private object intent, owner/channel validation, object metadata and leading-byte verification, one-use message attachment, size/count limits, byte progress and TUS resume/retry by upload-intent identity have developer TDD evidence | Provision/verify the private bucket; add explicit user cancel/pause UI, provider-backed expiry, robust orphan cleanup, avatar migration, full decoding/scanning and independent QA | Later unless secured | W09 |
