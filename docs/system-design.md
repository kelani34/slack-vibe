...
| Attachment | URL, MIME, name, size | Store object key and ownership/finalization state; signed URLs are derived, not canonical records |
...
| Upload intent/finalize | Channel, bounded metadata; then intent/object ID | Expiring upload grant; then finalized attachment record | Server verifies ownership and actual stored object |
