...
| QF26 | File attachments | F29 | Upload files with progress, cancellation and retry and attach them to a message | NOT RUN | Developer integration tests cover private signed upload intent, owner/channel scope, object size/MIME and signature verification, intent consumption, aggregate limits and failed-object cleanup; component tests cover the send retry lifecycle. The real private bucket, transfer progress/cancel/resume, provider/network/mobile behavior, full decoding/scanning and independent QA remain unverified |
