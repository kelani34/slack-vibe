...
| QF26 | File attachments | F29 | Upload files with progress, cancellation and retry and attach them to a message | NOT RUN | Developer integration tests cover private signed upload intent, workspace/channel scope, object size/MIME and signature verification, intent consumption and aggregate limits; unit/component tests cover TUS progress, pause, cancel/remove, same-intent retry/resume and accessible status. Cleanup route unit tests cover CRON_SECRET denial, 100-row bounded selection, one-hour expiry grace, unattached/private filtering, storage-before-database ordering and retryable provider/database failures; PostgreSQL integration verifies the expiry index. The private bucket/provider, live cron invocation, real network/mobile behavior, full decoding/scanning and independent QA remain unverified |
| QF27 | File preview and download | F30 | Preview supported images/files and download items the user may still access | NOT RUN | Developer tests cover authorized 5-minute preview/download grant issuance, denied/deleted/future items, path scope, loading/retry, and legacy URL compatibility; live bucket, expiration/revocation, device behavior and independent QA remain unverified |
