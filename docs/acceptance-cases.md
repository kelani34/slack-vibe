...
- Developer evidence: `tests/integration/upload-access.test.ts` covers owner/channel-bound signed intent creation, upload-denied membership, missing/incorrect size/MIME metadata, PNG signature mismatch/match, two-hour expiry and idempotent finalization. `tests/integration/message-access.test.ts` verifies forged URL rejection, server-owned attachment metadata, owner/channel/expiry/one-use checks, five-file and 25 MB aggregate limits. The browser transfer uses a private Supabase upload token, so file bytes avoid the Next/Vercel server-action body limit. `tests/integration/attachment-download.test.ts` and `src/components/message-item.test.tsx` cover expiring read grants and loading/retry states. Provider behavior remains mocked; bucket setup, progress/cancel/resume, cleanup, browser/device checks, full decode/scanning and independent QA remain NOT RUN.
