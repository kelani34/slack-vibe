...
- Developer evidence: `src/components/thread-sidebar.test.tsx` proves parent and reply queries remain disabled until the viewer identity is known; thread cache keys retain actor/workspace/resource scope. The paged root timeline query in `src/components/message-list.test.tsx` also remains disabled without viewer identity and becomes enabled when an actor is known. Live membership revocation and independent QA remain NOT RUN.
...
- **TC-F29-06** — Cleanup observes expiry grace and attachment references; concurrent or newly committed attachments are never deleted, and server memory remains bounded.
- **TC-F29-07** — Cleanup rejects missing/incorrect credentials, processes a bounded batch, removes storage before metadata, and preserves retry state after provider/database failure.
- Developer evidence: `tests/integration/upload-access.test.ts` covers owner/channel/workspace-bound signed intent creation, upload-denied membership, stale channel membership after workspace removal, missing/incorrect size/MIME metadata, PNG signature mismatch/match, two-hour expiry, same-intent grant renewal and idempotent finalization. `src/lib/private-file-upload.test.ts` covers direct TUS endpoint/token construction, 6 MB chunks, automatic retry configuration, intent-scoped fingerprint resume, byte progress and interrupted transfer handling; `src/hooks/use-send-message.test.tsx` covers pausing, removing the optimistic send without a false failure, retaining the intent and resuming; `src/components/message-input.test.tsx` checks accessible progress, pause, cancel/remove and retained file behavior. `src/app/api/cron/cleanup-uploads/route.test.ts` checks cron auth, 100-item cap, one-hour grace, unattached/private filtering, storage-before-database ordering and retryable failures. `tests/integration/upload-cleanup-index.test.ts` verifies the expiry index. `tests/integration/message-access.test.ts` verifies forged URL rejection, server-owned attachment metadata, owner/channel/expiry/one-use checks, five-file and 25 MB aggregate limits. Provider behavior remains mocked; bucket setup, live cron execution/provider cleanup, browser/device checks, full decode/scanning and independent QA remain NOT RUN.
...
- **TC-F43-03** — Actor/workspace cache keys scope profile, channel-member, timeline and thread projections; permission invalidation prevents stale private reuse.
