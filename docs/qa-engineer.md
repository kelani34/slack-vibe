...
Use the [108-requirement test matrix](test-matrix.md), [649 concrete cases](acceptance-cases.md) and [phase traceability](delivery-traceability.md) as the starting coverage map; do not treat one happy-path case as full feature verification. The [90-feature checklist](qa-checklist.md) supplies the user-facing completion view and separate route inventory.
