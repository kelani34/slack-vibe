...
**W09 status:** image/video/audio and PDF uploads now have bounded 12-byte signature checks, and SVG is rejected, with red/green integration evidence. This is a partial metadata-validation gate only; A06 remains open until private references/grants replace public URLs and the legacy avatar path, attachment finalization, non-image validation, bounded transfer and cleanup are verified.
