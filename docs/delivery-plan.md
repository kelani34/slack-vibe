...
**W09 status:** image uploads now have a bounded 12-byte prefix check and SVG is rejected, with red/green integration evidence. This is a partial metadata-validation gate only; A06 remains open until private references/grants replace public URLs and the legacy avatar path, attachment finalization, non-image validation, bounded transfer and cleanup are verified.
