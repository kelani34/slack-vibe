...
**W09 status:** image/video/audio and PDF uploads have bounded 12-byte signature checks, and SVG is rejected. A partial authorized-read path now supports optional object locators and 5-minute preview/download grants, with current channel/message checks and UI loading/retry coverage. The private bucket is not provisioned and uploads still persist public URLs; A06 remains open until private upload intents, finalization, legacy/avatar migration, complete validation, bounded transfer, expiry/revocation policy, and cleanup are implemented and verified.
